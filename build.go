...
var (
	...
	buildId               string   = "0"
	binaries              []string = []string{"galaxy-server", "galaxy-cli", "galaxy-reporter"}
	isDev                 bool     = false
...
func main() {
	...
	flag.BoolVar(&enterprise, "enterprise", enterprise, "Build enterprise version of Galaxy")
	...
	flag.BoolVar(&printGenVersion, "gen-version", printGenVersion, "generate Galaxy version and output (default: false)")
	flag.Parse()
	...
	for _, cmd := range flag.Args() {
		...
		case "build-cli":
			clean()
			build("galaxy-cli", "./galaxy-cli", []string{})

		case "build-server":
			clean()
			build("galaxy-server", "./galaxy-server", []string{})

		case "build":
			//clean()
			for _, binary := range binaries {
				build(binary, "./"+binary, []string{})
			}
...
func makeLatestDistCopies() {
	...
	latestMapping := map[string]string{
		"_amd64.deb":          "dist/galaxy_latest_amd64.deb",
		".x86_64.rpm":         "dist/galaxy-latest-1.x86_64.rpm",
		".linux-amd64.tar.gz": "dist/galaxy-latest.linux-x64.tar.gz",
		".linux-armv7.tar.gz": "dist/galaxy-latest.linux-armv7.tar.gz",
		".linux-armv6.tar.gz": "dist/galaxy-latest.linux-armv6.tar.gz",
		".linux-arm64.tar.gz": "dist/galaxy-latest.linux-arm64.tar.gz",
	}
...
func createDebPackages() {
	...
	createPackage(linuxPackageOptions{
		...
		homeDir:                "/usr/share/galaxy",
		binPath:                "/usr/sbin",
		configDir:              "/etc/galaxy",
		etcDefaultPath:         "/etc/default",
		etcDefaultFilePath:     "/etc/default/galaxy-server",
		initdScriptFilePath:    "/etc/init.d/galaxy-server",
		systemdServiceFilePath: "/usr/lib/systemd/system/galaxy-server.service",

		postinstSrc:    "packaging/deb/control/postinst",
		initdScriptSrc: "packaging/deb/init.d/galaxy-server",
		defaultFileSrc: "packaging/deb/default/galaxy-server",
		systemdFileSrc: "packaging/deb/systemd/galaxy-server.service",
...
func createRpmPackages() {
	...
	createPackage(linuxPackageOptions{
		...
		etcDefaultFilePath:     "/etc/sysconfig/galaxy-server",
		initdScriptFilePath:    "/etc/init.d/galaxy-server",
		systemdServiceFilePath: "/usr/lib/systemd/system/galaxy-server.service",

		postinstSrc:    "packaging/rpm/control/postinst",
		initdScriptSrc: "packaging/rpm/init.d/galaxy-server",
		defaultFileSrc: "packaging/rpm/sysconfig/galaxy-server",
		systemdFileSrc: "packaging/rpm/systemd/galaxy-server.service",
...
func createPackage(options linuxPackageOptions) {
	packageRoot, _ := ioutil.TempDir("", "galaxy-linux-pack")
	...
	runPrint("mkdir", "-p", filepath.Join(packageRoot, "/var/lib/galaxy"))
	...
	// copy plugins files
	runPrint("cp", "-R", filepath.Join(workingDir, "tmp/data/plugins"), filepath.Join(packageRoot, "/var/lib/galaxy/"))
	// remove bin path
	...
	if enterprise {
		name += "-enterprise"
		args = append(args, "--replaces", "galaxy")
	}
...
func setup() {
	runPrint("go", "install", "-v", "./galaxy-server")
}
...
func clean() {
	...
	rmr("dist")
	rmr("tmp")
	rmr(filepath.Join(os.Getenv("GOPATH"), fmt.Sprintf("pkg/%s_%s/github.com/galaxy", goos, goarch)))
}
